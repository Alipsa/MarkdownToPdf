# MarkdownToPdf - a pdf creation library converting Freemarker markup to pdf
...
[![javadoc](https://javadoc.io/badge2/se.alipsa/journo-runtime/javadoc.svg)](https://javadoc.io/doc/se.alipsa/MarkdownToPdf)
...
You can configure Markdown parsing/rendering when creating the engine:
```java
...
For Markdown files, relative resources are resolved from the Markdown file's
directory:
```java
byte[] pdf = new Md2PdfEngine()
    .markdown(Path.of("reports/report.md"))
    .toPdf();
```

For Markdown strings, set the base path explicitly when using relative images
or CSS URLs:
```java
byte[] pdf = new Md2PdfEngine()
    .markdown("# Report\n\n![Logo](logo.png)")
...
JournoEngine engine = new JournoEngine(this, "/style");
Map<String, Object> data = new HashMap<>();
...
A simple page header and footer can be added directly from the fluent API:
```java
byte[] pdf = new Md2PdfEngine()
    ...
    .toPdf();
```
If you omit `pageMargins(...)`, md2pdf uses `0.75in` when a header or footer is present.
...
```groovy
String externalCssPath = this.getClass().getResource("/style/mystyle.css").toExternalForm();
...
## Styling
Use `css(...)` when you want to replace the default stylesheet completely:
```java
byte[] pdf = new Md2PdfEngine()
    .markdown("# Report")
...
Use `addCss(...)` when you want to keep the default Markdown styling and add your own overrides:
```java
byte[] pdf = new Md2PdfEngine()
    .markdown("# Report")
...
## PDF metadata
PDF document metadata can be set on the fluent job:
```java
byte[] pdf = new Md2PdfEngine()
...
## Fonts
You can explicitly register fonts on the fluent API and then refer to the family
from CSS:
```java
File jersey = new File("src/main/resources/fonts/Jersey25-Regular.ttf");
byte[] pdf = new Md2PdfEngine()
    .markdown("# Font Test\n\nCustom font text")
    .css("body { font-family: \"Jersey 25\"; }")
    .font(jersey, "Jersey 25")
    .toPdf();
```
